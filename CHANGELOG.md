...
### Changed
- **TV DR dashboard fullscreen**: Keep left card text large in fullscreen mode
  - Fullscreen card COM, info, comment, and timer sizes no longer shrink
  - Comments enlarged for better long-distance readability
...
### Fixed
- **SSRS PowerShell prompt**: Suppress the PowerShell 5.1 "Script Execution Risk" prompt during report downloads
  - Added `-UseBasicParsing` for `Invoke-WebRequest` when running on PowerShell 5.1
  - Added a PowerShell version guard so PowerShell 7+ skips `-UseBasicParsing`

### Documentation
- **DATA_SYNC_EXPLANATION.txt**: Simplified the data sync explanation for non-technical users
